...
//OTHER VARIANT
    public int ask(boolean lastCorrect, Card card, ArrayList<Card> options) {
        ...
        if(lastCorrect) {
            ...
            lastPerceivedRule.add(new Guess(oldRule,false));
            if(!lastPerceivedRule.get(lastPerceivedRule.size()-2).isWasCorrect()){
                switch(lastPerceivedRule.get(lastPerceivedRule.size()-1).getRule()) {
                    case COLOR:
                        if(lastPerceivedRule.get(lastPerceivedRule.size()-2).getRule() == Match.SHAPE) {
                            perceivedRule = Match.NUMBER;
                        } else {
                            perceivedRule = Match.SHAPE;
                        }
                        break;
                    case SHAPE:
                        if(lastPerceivedRule.get(lastPerceivedRule.size()-2).getRule() == Match.NUMBER) {
                            perceivedRule = Match.COLOR;
                        } else {
                            perceivedRule = Match.NUMBER;
                        }
                        break;
                    case NUMBER:
                        if(lastPerceivedRule.get(lastPerceivedRule.size()-2).getRule() == Match.COLOR) {
                            perceivedRule = Match.SHAPE;
                        } else {
                            perceivedRule = Match.COLOR;
                        }
